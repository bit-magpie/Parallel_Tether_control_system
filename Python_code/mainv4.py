...
def update_text():
    st.session_state.lngValue = str(st.session_state.slider)
    
def update_slider():
    st.session_state.slider = int(st.session_state.lngValue)
...
if len(port_list) > 0:
    ...
    st.markdown("""---""")
    st.slider("Value", -3500, 3500, 0, key="slider", on_change=update_text)
    btncol1, btncol2, coltxt, btncol3 = st.columns((1,1,2,1))       
    btn_tq_on = btncol1.button("Torque on", key="btnTqOn")
    btn_tq_off = btncol2.button("Release", key="btnTqOf")
    coltxt.text_input("Value", key="lngValue", on_change=update_slider, label_visibility="collapsed")
    ...
    if btn_set_length:
        for k in available_reels:
            ...
            length = st.session_state["lngValue"]
            if chk_value:
                reel_list[k].setValue(int(length))
